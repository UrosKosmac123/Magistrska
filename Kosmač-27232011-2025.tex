...
je bolj smiselno namesto mase obravnavati gostoto $\rho$, zato enačbo \ref{zgoraj} delimo z volumnom domene Vol$(\Omega)$
in vstavimo v ohranitveno enačbo vstavimo $\bd{f} = \rho\bd{u}$ in označimo $\bd{\text{$\gamma$}} = \bd{F}$, kjer je $\bd{F}$ vsota vseh sil na enoto volumna $\Omega$. Dobimo
\begin{equation}
...
\end{equation}
Dobili smo 2. Newtonov zakon na enoto volumna.
...
$D(\bd{a}, r)$ za $r>0$, odprta krogla točke $\bd{a}$. Označimo z $\bd{n}(\bd{a})$ enotsko normalo na $\partial \Omega$ v točki $\bd{a}$, ki kaže izven volumna $\Omega$.
...
\item $\forall \bd{x} \in \Omega$ in $i=1, \dots, n$: $||\bd{U}(\bd{x}, t)|| < G(\bd{r}, \bd{x}), ||\frac{\partial \bd{U}}{\partial t}(\bd{x}, t)|| < G(\bd{r}, \bd{x}), \\
||\frac{\partial \bd{U}}{\partial x_i}(\bd{x}, t)|| < G(\bd{r}, \bd{x})$.
\end{itemize}
...
\end{align*}
Če želimo, da je $G$ idempotent, mora veljati $\overline{\overline{{U}}} = \overline{{U}}$. 
\begin{align*}
...
$\kappa_n$, ki jo imenujemo $n$-to valovno število. To bo ključno za reševanje 
...
Filtriramo vsako enačbo posebej, kjer upoštevamo, da filtracija komutira z odvajanjem. Torej
\begin{align*}
...
Iz definicije tenzorja $\tau^R$ dodamo še dve definiciji
\begin{definicija}
...
Definicija pove, da  tenzor $\tau^R$ razcepimo na 
...
Zaradi hipoteze o lokalni izotropiji, lahko kovariančno matriko zapišemo
...
$$
Privzeli bomo, da je $m\geq r$, kar pomeni, da imamo več neodvisnih spremen-ljivk kot enot. Sedaj lahko za $k > r$ enoto $R_k$ izrazimo kot 
$$
...
\textbf{konstanta}. Njena vrednost je določena eksperimentalno
...
% Poglejmo si skico \ref{boussinesq2}
% \begin{figure}[h!]
%   \begin{center}
%     \begin{tikzpicture}
...
Sledeče analiza bo narejena za statistično povprečje, sklep pa bo sledil tudi za filtracijo. 
$$
...
\noindent
Analogno z enačbo za strižno napetost pri Newtonovih tekočinah \ref{tenzor_tekocine}
...
modelov, ki si jih ogledamo v naslednjem razdelku.
...
uporabe visoko ločljivostnih turbulentnega modela za slovenske 
...
predstavljajo pomemben korak naprej. Model MicroHH se je 
